...
public class ConditionUI {
    ...
    public static JPanel getMustBeConsecitiveConditionUI(ConditionDescription desc) {
        ...
        if (desc.subs.size() == 0) {
            desc.subs.add(new SubjectPlaceHolder());

        }
        if (subject1.getSelectedIndex() != -1) {
            if (desc.subs.size() != 0) {
                subject1.setSelectedItem(desc.subs.get(0).getSubject());
            } else {
                desc.subs.set(0, new SubjectPlaceHolder(subject1.getItemAt(0).toString(),
                        GlobalSpace.classController.getTeacher(desc.clas, subject1.getItemAt(0).toString())));
            }
        }
        ...
        subject1.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                desc.subs.set(0, new SubjectPlaceHolder(subject1.getSelectedItem().toString(),
                        GlobalSpace.classController.getTeacher(desc.clas, subject1.getSelectedItem().toString())));
            }
        });

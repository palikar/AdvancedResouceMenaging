...
public class ClassNode {
    ...
    public ArrayList<HourNode> ndoes;
    public String name;
    public Map<SubjectPlaceHolder, Integer> subjectsPlan;
    public Map<SubjectPlaceHolder, Integer> currentOccurence;
    ...
    public void addCondition(ConditionDescription con) {
        switch (con.code) {
            case ConditionCodeList.mustBeConsecitiveConditionCode:
                for (int i = 0; i < 5; i++) {
                    for (int j = 0; j < 7; j++) {
                        if (con.daysEnebled[i][j] == 1) {
                            days.get(i).getHours().get(j).setConsecitiveCondition(con.subs.get(0));
                        }
                    }
                }
                break;

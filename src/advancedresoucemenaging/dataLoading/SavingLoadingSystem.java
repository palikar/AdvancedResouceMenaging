...
public class SavingLoadingSystem
{
            ...
            while (!(line = bufferedReader.readLine()).equals("end!"))
            {
                ...
                while (!(line = bufferedReader.readLine()).equals("newClass!"))
                {
                    ...
                    for (int i = 0; i < subjects.length; i++)
                    {
                        ...
                        GlobalSpace.classController.classes.get(clas).schedule[i][index] = subjectPlaceHolder;
                    }
                    index++;
                }
    ...
    public static void saveGlobalSpace(File file)
    {
        try
        {
            ...
            {
                bufferedWriter.write(clas.name);
                bufferedWriter.newLine();
            }
            bufferedWriter.write("end!");
            bufferedWriter.newLine();
            ...
            for (Entry<String, Class> clas : GlobalSpace.classController.classes.entrySet())
            {
                bufferedWriter.write(clas.getKey());
                bufferedWriter.newLine();
                for (Entry<SubjectPlaceHolder, Integer> subject : clas.getValue().subjectPlan.entrySet())
                {
            ...
            for (Entry<String, Class> clas : GlobalSpace.classController.classes.entrySet())
            {
                ...
                ArrayList<ConditionDescription> cons = clas.getValue().conditions;
            ...
            for (Entry<String, Class> clas : GlobalSpace.classController.classes.entrySet())
            {
                ...
                SubjectPlaceHolder[][] schedule = clas.getValue().schedule;
    ...
    public static void saveTeachersScheduleToPdf(File f)
    {
        try
        {
            ...
            int entiesOnPage = 0, tablesPerPage = 2;
            for (String teacher : GlobalSpace.teacherController.teachers)
            {
                entiesOnPage++;
                ...
                for (int i = 0; i < 7; i++)
                {
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        ...
                        for (Entry<String, Class> clas : GlobalSpace.classController.classes.entrySet())
                        {
                            if (clas.getValue().schedule[j][i] == null || clas.getValue().schedule[j][i].getSubject().endsWith("unknown"))
                            {
                                continue;
                            }
                            if (clas.getValue().schedule[j][i].getTeacher().equals(teacher))
                            {
